...
"""
================================================================================
《GGBOM: 终末医疗兵》双层全景地图系统与 100% 完整视野总装
- 双层地图：Ground (底层 Priority 0) + Overhead (顶层遮挡 Priority 1000)
- 正交相机精确对齐 941x1672 原生尺寸，全景 100% 完整显示无裁剪
- 纯数学流真·8 向全向平滑移动与单体 Pawn 架构
- 2D 色彩 100% 原画直通保真
================================================================================
"""
...
def log(msg: str):
    unreal.log(f"[GGBOM-DualLayerMap] {msg}")
...
def build_pristine_medic_pawn():
    log("🔨 构建单体 100% 纯净 BP_Player_Medic Pawn...")
    ...
    for comp in cdo.get_components_by_class(unreal.ActorComponent):
        if isinstance(comp, unreal.PaperFlipbookComponent):
            has_fb = True
        elif isinstance(comp, unreal.BoxComponent):
            has_col = True
    ...
    log("✅ 纯净单体 BP_Player_Medic 构建成功！")
    return bp

def assemble_master_game():
    log("🚀 组装双层地图 (Ground + Overhead) 与 100% 完整视野战场...")
    ...
    player_bp = build_pristine_medic_pawn()
    ...
    log("🎉 MAP_GGBOM_Main 双层全景地图与 100% 完整视野保存完成！")

...
"""
================================================================================
《GGBOM: 终末医疗兵》全量战斗与运动手感终极调优:
1. 斜向移动速度归一化 (Normalize): 解决斜向移动速度超快问题 (恒定 450 uu/s)
2. 子弹速度降低至 600 uu/s (慢速动能弹，清晰可见，极大方便截图与动能轨迹观测)
3. 彻底根治朝向偏向 Bug: 建立单向流状态机，射击 100% 紧贴枪口与最后移动朝向
4. 规范开火冷却与 8 向精准攻击动作匹配
================================================================================
"""
...
def calibrate_slow_bullet():
    print("🚀 1. 降低子弹速度至 600 uu/s (慢速动能弹，便于截图与动效展示)...")
    ...
    for h in handles:
        ...
        elif vname == "BulletFlipbook":
            obj.set_editor_property("relative_scale3d", unreal.Vector(0.42, 0.42, 0.42))
    ...
    print("✅ BP_ProjectileBase 600 uu/s 慢速子弹配置完成！")

def build_calibrated_player():
    print("🎮 2. 重构主角 8 向斜向归一化与 100% 同步射击图表...")
    ...
    # 2. 向量归一化 (Normalize 2D)，解决斜向移动超快问题！
    ...
    norm_vec = fn(ed, "/Script/Engine.KismetMathLibrary.Normal", 1080, 0)
    connect(raw_vec, "ReturnValue", norm_vec, "A")
    
    mul_spd = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1280, 0)
    connect(norm_vec, "ReturnValue", mul_spd, "A"); set_value(mul_spd, "B", 450.0) # 恒定 450 uu/s
    
    mul_dt = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1480, 0)
    ...
    # ScaleX 设置 (向右时 -0.45 镜像，向左/正向时 0.45)
    ...
    # 向上慢速动能子弹: 枪口 (X, 0, Z+45), Pitch=90.0
    add_up = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 3020, -260); connect(get_loc, "ReturnValue", add_up, "A"); set_value(add_up, "B", "0,0,45")
    rot_up = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3020, -140); set_value(rot_up, "Pitch", 90.0); set_value(rot_up, "Yaw", 0.0); set_value(rot_up, "Roll", 0.0)
    ...
    add_down = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 3240, -100); connect(get_loc, "ReturnValue", add_down, "A"); set_value(add_down, "B", "0,0,-45")
    rot_down = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3240, 20); set_value(rot_down, "Pitch", -90.0); set_value(rot_down, "Yaw", 0.0); set_value(rot_down, "Roll", 0.0)
    ...
    pitch_val_side = fn(ed, "/Script/Engine.KismetMathLibrary.SelectFloat", 3240, 280); set_value(pitch_val_side, "A", 180.0); set_value(pitch_val_side, "B", 0.0); connect(is_right_dir, "ReturnValue", pitch_val_side, "bPickA")
    ...
    rot_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3680, 160); connect(pitch_val_side, "ReturnValue", rot_side, "Pitch"); set_value(rot_side, "Yaw", 0.0); set_value(rot_side, "Roll", 0.0)
    ...
    is_curr_up = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3020, 700); connect(get_curr_fb, "ReturnValue", is_curr_up, "A")
    if fb_run_up:
        set_value(is_curr_up, "B", f"PaperFlipbook'{fb_run_up.get_path_name()}'")
    br_idle_up = ed.add_branch_node(); br_idle_up.set_node_pos(unreal.IntPoint(3240, 700))
    connect(br_move_state, "else", br_idle_up, "execute"); connect(is_curr_up, "ReturnValue", br_idle_up, "Condition")
    
    set_fb_idle_up = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3460, 700)
    if fb_idle_up:
    ...
    is_curr_side = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3240, 800); connect(get_curr_fb, "ReturnValue", is_curr_side, "A")
    if fb_run_left:
        set_value(is_curr_side, "B", f"PaperFlipbook'{fb_run_left.get_path_name()}'")
    br_idle_side = ed.add_branch_node(); br_idle_side.set_node_pos(unreal.IntPoint(3460, 800))
    connect(br_idle_up, "else", br_idle_side, "execute"); connect(is_curr_side, "ReturnValue", br_idle_side, "Condition")
    
    set_fb_idle_side = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3680, 800)
    if fb_idle_left:
    ...
    set_fb_idle_down = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3680, 900)
    if fb_idle_down:
    ...
    BPLIB.compile_blueprint(bp)
    ASSETS.save_loaded_asset(bp, only_if_is_dirty=False)
    print("✅ 主角蓝图 8 向斜向归一化与精确朝向射击编译完成！")

def main():
    calibrate_slow_bullet()
    build_calibrated_player()

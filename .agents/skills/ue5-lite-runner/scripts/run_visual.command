...
ARGS=("$PROJECT_FILE")
if [ -n "$START_MAP" ]; then ARGS+=("$START_MAP"); fi
...
ARGS+=(
  ...
  "-ResY=960"
  "-NoSplash"
  "-ExecCmds=sg.ShadowQuality 0,sg.PostProcessQuality 0,t.MaxFPS 60"
)

echo "MODE=VISUAL_GAME"
echo "PROJECT=$PROJECT_FILE"
echo "MAP=${START_MAP:-<GameDefaultMap>}"
echo "WINDOW=540x960"
echo "FPS_CAP=60"

UE_APP="$ENGINE_ROOT/Engine/Binaries/Mac/UnrealEditor.app"

if [ -d "$UE_APP" ]; then
  open -n -a "$UE_APP" --args "${ARGS[@]}"
  sleep 2
  PID="$(pgrep -f "UnrealEditor.*$PROJECT_FILE" | head -n 1 || true)"
  if [ -n "$PID" ]; then
    echo "$PID" > "$PID_FILE"
    echo "LITE_RUN_STATUS=STARTED"
    echo "PID=$PID"
    echo "PID_FILE=$PID_FILE"
    echo "LAUNCH_MODE=MACOS_APP_BUNDLE"
    exit 0
  fi
fi

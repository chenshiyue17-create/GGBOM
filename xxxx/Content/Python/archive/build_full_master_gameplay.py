...
"""
================================================================================
《GGBOM: 终末医疗兵》完美视角锁定 + 8 向动态动作与转向 + 攻击技能总装
================================================================================
"""
...
def log(msg: str):
    unreal.log(f"[GGBOM-MasterFix] {msg}")
...
def build_perfect_wasd_player_bp() -> unreal.Blueprint:
    log("🔨 构建包含绝对固定 9:16 全局摄像机的可操控 Pawn (BP_GGBOM_Player)...")
    bp_path = f"{GEN}/Blueprints/BP_GGBOM_Player"
    ...
    # 3. 绝对锁定全局 9:16 正交摄像机 (Absolute Location & Rotation)
    camera = add_component(bp, "Camera", unreal.CameraComponent.static_class())
    camera.set_editor_property("absolute_location", True)
    camera.set_editor_property("absolute_rotation", True)
    camera.set_editor_property("relative_location", unreal.Vector(0.0, -500.0, 0.0))
    ...
    directions = [
        (br_a, 520.0, 0.0, 0.0, -220),   # A: 左移 (+X)
        (br_d, -520.0, 0.0, 0.0, -100),  # D: 右移 (-X)
        (br_w, 0.0, 0.0, 520.0, 20),    # W: 上移 (+Z)
        (br_s, 0.0, 0.0, -520.0, 140),  # S: 下移 (-Z)
    ]
    ...
    log("✅ BP_GGBOM_Player 绝对锁定 9:16 全局摄像机 Pawn 编译完成！")
    return bp

def assemble_master_stage():
    log("🚀 组装主关卡 MAP_GGBOM_Main (完美全景 9:16 + 1:1 HUD + WASD Pawn)...")
    ...
    # 先编译拥有绝对 9:16 全景相机的 Player Pawn
    player_bp = build_perfect_wasd_player_bp()
    ...
    # 单一唯一可控主角 (Screen: 540, 1560) -> UE (0, -10, -600)
    p_pos = to_ue_pos(540, 1560, -10)
    player_cls = unreal.load_class(None, f"{GEN}/Blueprints/BP_GGBOM_Player.BP_GGBOM_Player_C")
    if player_cls:
        player_pawn = unreal.EditorLevelLibrary.spawn_actor_from_class(player_cls, p_pos, unreal.Rotator())
        if player_pawn:
            player_pawn.set_actor_label("BP_GGBOM_Player_Playable")
            player_pawn.set_actor_scale3d(unreal.Vector(0.65, 0.65, 0.65))
            player_pawn.set_editor_property("auto_possess_player", unreal.AutoReceiveInput.PLAYER0)
            log("🎉 唯一可操控 WASD 主角实体生成完成！")
    ...
    log("🎉 主关卡 MAP_GGBOM_Main 全量保存完成！")

if __name__ == "__main__":
    assemble_master_stage()

...
"""
================================================================================
《GGBOM: 终末医疗兵》子弹 2D 平面精准旋转与朝向保持终极解决方案
================================================================================
"""
...
def setup_projectile():
    print("🚀 1. 设置 BP_ProjectileBase 尺寸 (0.13) 与 600 uu/s 动能速度...")
    ...
    print("✅ BP_ProjectileBase 完成！")

def setup_player():
    print("🎮 2. 重构主角朝向持久锁定与 8 向旋转射击...")
    ...
    # 向量归一化
    raw_vec = fn(ed, "/Script/Engine.KismetMathLibrary.MakeVector", 880, 0)
    connect(raw_x, "ReturnValue", raw_vec, "X"); set_value(raw_vec, "Y", 0.0); connect(raw_z, "ReturnValue", raw_vec, "Z")
    norm_vec = fn(ed, "/Script/Engine.KismetMathLibrary.Normal", 1080, 0); connect(raw_vec, "ReturnValue", norm_vec, "A")
    mul_spd = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1280, 0); connect(norm_vec, "ReturnValue", mul_spd, "A"); set_value(mul_spd, "B", 450.0)
    mul_dt = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1480, 0); connect(mul_spd, "ReturnValue", mul_dt, "A"); connect(tick, "DeltaSeconds", mul_dt, "B")
    
    move_node = fn(ed, "/Script/Engine.Actor.K2_AddActorWorldOffset", 1700, 0)
    ...
    get_comp = fn(ed, "/Script/Engine.Actor.GetComponentByClass", 1920, -200); set_value(get_comp, "ComponentClass", "Class'/Script/Paper2D.PaperFlipbookComponent'")
    
    # 采样 J 键
    key_j = fn(ed, "/Script/Engine.PlayerController.IsInputKeyDown", 1920, 200); set_value(key_j, "Key", "J"); connect(pc_tick, "ReturnValue", key_j, "self")
    br_is_shooting = ed.add_branch_node(); br_is_shooting.set_node_pos(unreal.IntPoint(2140, 0))
    connect(move_node, "then", br_is_shooting, "execute")
    connect(key_j, "ReturnValue", br_is_shooting, "Condition")

    # 朝向特征
    is_up_dir = fn(ed, "/Script/Engine.KismetMathLibrary.Greater_DoubleDouble", 2360, -260); connect(raw_z, "ReturnValue", is_up_dir, "A"); set_value(is_up_dir, "B", 0.0)
    is_down_dir = fn(ed, "/Script/Engine.KismetMathLibrary.Less_DoubleDouble", 2360, -160); connect(raw_z, "ReturnValue", is_down_dir, "A"); set_value(is_down_dir, "B", 0.0)
    is_right_dir = fn(ed, "/Script/Engine.KismetMathLibrary.Less_DoubleDouble", 2360, 40); connect(raw_x, "ReturnValue", is_right_dir, "A"); set_value(is_right_dir, "B", 0.0)

    # ScaleX 镜像控制 (向右时 -0.45, 向左/正向时 0.45)
    scale_val_x = fn(ed, "/Script/Engine.KismetMathLibrary.SelectFloat", 2580, -320); set_value(scale_val_x, "A", -0.45); set_value(scale_val_x, "B", 0.45); connect(is_right_dir, "ReturnValue", scale_val_x, "bPickA")
    scale_vec = fn(ed, "/Script/Engine.KismetMathLibrary.MakeVector", 2800, -320); connect(scale_val_x, "ReturnValue", scale_vec, "X"); set_value(scale_vec, "Y", 0.45); set_value(scale_vec, "Z", 0.45)
    set_sc = fn(ed, "/Script/Engine.SceneComponent.SetRelativeScale3D", 3020, -320); connect(get_comp, "ReturnValue", set_sc, "self"); connect(scale_vec, "ReturnValue", set_sc, "NewScale3D")

    # ==================== 射击分支 (Shooting = True) ====================
    br_shoot_up = ed.add_branch_node(); br_shoot_up.set_node_pos(unreal.IntPoint(2580, -200))
    connect(br_is_shooting, "then", br_shoot_up, "execute"); connect(is_up_dir, "ReturnValue", br_shoot_up, "Condition")

    # 1. 向上射击: 动作 fb_atk_up, 枪口 (X, 0, Z+40), 子弹朝向 Roll=90°
    set_fb_atk_up = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2800, -260)
    ...
    get_loc = fn(ed, "/Script/Engine.Actor.K2_GetActorLocation", 2800, 0)
    add_up = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 3020, -260); connect(get_loc, "ReturnValue", add_up, "A"); set_value(add_up, "B", "0,0,40")
    rot_up = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3020, -140); set_value(rot_up, "Pitch", 0.0); set_value(rot_up, "Yaw", 0.0); set_value(rot_up, "Roll", 90.0)
    trans_up = fn(ed, "/Script/Engine.KismetMathLibrary.MakeTransform", 3240, -200); connect(add_up, "ReturnValue", trans_up, "Location"); connect(rot_up, "ReturnValue", trans_up, "Rotation"); set_value(trans_up, "Scale", "1,1,1")
    spawn_up = fn(ed, "/Script/Engine.GameplayStatics.BeginDeferredActorSpawnFromClass", 3460, -260)
    ...
    finish_up = fn(ed, "/Script/Engine.GameplayStatics.FinishSpawningActor", 3720, -260)
    ...
    # 2. 向下射击: 动作 fb_atk_down, 枪口 (X, 0, Z-40), 子弹朝向 Roll=-90°
    br_shoot_down = ed.add_branch_node(); br_shoot_down.set_node_pos(unreal.IntPoint(2800, -60))
    connect(br_shoot_up, "else", br_shoot_down, "execute"); connect(is_down_dir, "ReturnValue", br_shoot_down, "Condition")

    set_fb_atk_down = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3020, -100)
    ...
    add_down = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 3240, -100); connect(get_loc, "ReturnValue", add_down, "A"); set_value(add_down, "B", "0,0,-40")
    rot_down = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3240, 20); set_value(rot_down, "Pitch", 0.0); set_value(rot_down, "Yaw", 0.0); set_value(rot_down, "Roll", -90.0)
    trans_down = fn(ed, "/Script/Engine.KismetMathLibrary.MakeTransform", 3460, -40); connect(add_down, "ReturnValue", trans_down, "Location"); connect(rot_down, "ReturnValue", trans_down, "Rotation"); set_value(trans_down, "Scale", "1,1,1")
    spawn_down = fn(ed, "/Script/Engine.GameplayStatics.BeginDeferredActorSpawnFromClass", 3680, -100)
    ...
    finish_down = fn(ed, "/Script/Engine.GameplayStatics.FinishSpawningActor", 3940, -100)
    ...
    # 3. 侧向射击: 动作 fb_atk_side, 枪口 (+40 或 -40), 子弹朝向 Roll=0° (左) 或 180° (右)
    set_fb_atk_side = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3020, 160)
    ...
    roll_val_side = fn(ed, "/Script/Engine.KismetMathLibrary.SelectFloat", 3240, 280); set_value(roll_val_side, "A", 180.0); set_value(roll_val_side, "B", 0.0); connect(is_right_dir, "ReturnValue", roll_val_side, "bPickA")
    offset_val_side = fn(ed, "/Script/Engine.KismetMathLibrary.SelectFloat", 3240, 380); set_value(offset_val_side, "A", -40.0); set_value(offset_val_side, "B", 40.0); connect(is_right_dir, "ReturnValue", offset_val_side, "bPickA")
    
    offset_vec_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeVector", 3460, 380); connect(offset_val_side, "ReturnValue", offset_vec_side, "X"); set_value(offset_vec_side, "Y", 0.0); set_value(offset_vec_side, "Z", 0.0)
    add_side = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 3680, 280); connect(get_loc, "ReturnValue", add_side, "A"); connect(offset_vec_side, "ReturnValue", add_side, "B")
    
    rot_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 3680, 160); set_value(rot_side, "Pitch", 0.0); set_value(rot_side, "Yaw", 0.0); connect(roll_val_side, "ReturnValue", rot_side, "Roll")
    trans_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeTransform", 3900, 220); connect(add_side, "ReturnValue", trans_side, "Location"); connect(rot_side, "ReturnValue", trans_side, "Rotation"); set_value(trans_side, "Scale", "1,1,1")
    spawn_side = fn(ed, "/Script/Engine.GameplayStatics.BeginDeferredActorSpawnFromClass", 4120, 160)
    ...
    finish_side = fn(ed, "/Script/Engine.GameplayStatics.FinishSpawningActor", 4380, 160)
    ...
    br_move_state = ed.add_branch_node(); br_move_state.set_node_pos(unreal.IntPoint(2580, 480))
    ...
    # 移动分支
    br_side_run = ed.add_branch_node(); br_side_run.set_node_pos(unreal.IntPoint(3020, 480))
    connect(set_sc, "then", br_side_run, "execute"); connect(cmp_x, "ReturnValue", br_side_run, "Condition")
    
    set_fb_run_side = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3240, 440)
    ...
    br_up_run = ed.add_branch_node(); br_up_run.set_node_pos(unreal.IntPoint(3240, 560))
    connect(br_side_run, "else", br_up_run, "execute"); connect(is_up_dir, "ReturnValue", br_up_run, "Condition")
    
    set_fb_run_up = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3460, 520)
    ...
    set_fb_run_down = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3460, 620)
    ...
    # 待机保持分支: 严格判定当前 Flipbook，如果是 Run_Up 或 Idle_Up 则继续 Idle_Up；如果是 Run_Left 或 Idle_Left 则继续 Idle_Left；其余为 Idle_Down
    get_curr_fb = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.GetFlipbook", 2800, 700); connect(get_comp, "ReturnValue", get_curr_fb, "self")
    
    is_up_r = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3020, 680); connect(get_curr_fb, "ReturnValue", is_up_r, "A")
    if fb_run_up: set_value(is_up_r, "B", f"PaperFlipbook'{fb_run_up.get_path_name()}'")
    is_up_i = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3020, 760); connect(get_curr_fb, "ReturnValue", is_up_i, "A")
    if fb_idle_up: set_value(is_up_i, "B", f"PaperFlipbook'{fb_idle_up.get_path_name()}'")
    is_keep_up = fn(ed, "/Script/Engine.KismetMathLibrary.BooleanOR", 3240, 720); connect(is_up_r, "ReturnValue", is_keep_up, "A"); connect(is_up_i, "ReturnValue", is_keep_up, "B")
    
    br_idle_up = ed.add_branch_node(); br_idle_up.set_node_pos(unreal.IntPoint(3460, 700))
    connect(br_move_state, "else", br_idle_up, "execute"); connect(is_keep_up, "ReturnValue", br_idle_up, "Condition")
    
    set_fb_idle_up = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3680, 700)
    ...
    is_side_r = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3240, 840); connect(get_curr_fb, "ReturnValue", is_side_r, "A")
    if fb_run_left: set_value(is_side_r, "B", f"PaperFlipbook'{fb_run_left.get_path_name()}'")
    is_side_i = fn(ed, "/Script/Engine.KismetMathLibrary.EqualEqual_ObjectObject", 3240, 920); connect(get_curr_fb, "ReturnValue", is_side_i, "A")
    if fb_idle_left: set_value(is_side_i, "B", f"PaperFlipbook'{fb_idle_left.get_path_name()}'")
    is_keep_side = fn(ed, "/Script/Engine.KismetMathLibrary.BooleanOR", 3460, 880); connect(is_side_r, "ReturnValue", is_keep_side, "A"); connect(is_side_i, "ReturnValue", is_keep_side, "B")
    
    br_idle_side = ed.add_branch_node(); br_idle_side.set_node_pos(unreal.IntPoint(3680, 800))
    connect(br_idle_up, "else", br_idle_side, "execute"); connect(is_keep_side, "ReturnValue", br_idle_side, "Condition")
    
    set_fb_idle_side = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3900, 800)
    ...
    set_fb_idle_down = fn(ed, "/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 3900, 900)

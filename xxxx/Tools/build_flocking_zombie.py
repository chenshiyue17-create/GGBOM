# -*- coding: utf-8 -*-
"""
build_flocking_zombie.py
普通行尸（BP_Enemy_ZombieWalker）扇面内侧多路分流（Surround Slot = 40.0 uu）与防重叠排斥构建
"""
import unreal
from pathlib import Path

ROOT = Path(unreal.Paths.project_dir()).resolve()
OUT = ROOT / "output/flocking_zombie_result.txt"
...
def log(msg):
    unreal.log(f"[ZOMBIE_FLOCKING] {msg}")
    print(f"[ZOMBIE_FLOCKING] {msg}", flush=True)

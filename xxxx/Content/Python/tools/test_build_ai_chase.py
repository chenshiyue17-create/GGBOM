...
if not bp:
    raise RuntimeError("Cannot load BP_Enemy_ZombieWalker")
...
# 2. 检查或设置 MoveSpeed 变量
real_type = bplib.get_basic_type_by_name("real")
var_names = [str(x) for x in bplib.list_member_variable_names(bp, False)]
if "MoveSpeed" not in var_names:
    graph = bplib.find_event_graph(bp)
    ed = unreal.BlueprintGraphEditor.get_graph_editor(graph)
    ed.add_member_variable("MoveSpeed", real_type, "65.0")
...
# 3. 创建 ChasePlayer 函数图表
func_name = "ChasePlayer"
if func_name in [str(x) for x in bplib.list_graph_names(bp)]:
    bplib.remove_function_graph(bp, func_name)

ed = unreal.BlueprintGraphEditor.create_and_edit_function_graph(bp, func_name)
dt_param = ed.add_graph_input_parameter("DeltaSeconds", real_type, "0.0")
entry_pin = ed.find_graph_entry_pin()
...
make_v = fn(ed, "/Script/Engine.KismetMathLibrary.MakeVector", 1360, 100)
connect(break_v, "X", make_v, "X")
set_value(make_v, "Y", 0.0)
connect(break_v, "Z", make_v, "Z")

# 乘以 MoveSpeed
get_speed = place(ed.add_get_member_variable_node("MoveSpeed"), 1360, 240)
mul_speed = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1580, 100)
connect(make_v, "ReturnValue", mul_speed, "A")
connect(get_speed, "MoveSpeed", mul_speed, "B")

# 乘以 DeltaSeconds
mul_dt = fn(ed, "/Script/Engine.KismetMathLibrary.Multiply_VectorFloat", 1800, 100)
connect(mul_speed, "ReturnValue", mul_dt, "A")
connect(entry_pin.get_owning_node(), "DeltaSeconds", mul_dt, "B")

# 执行位移 (bSweep = True)
move_node = fn(ed, "/Script/Engine.Actor.K2_AddActorWorldOffset", 2050, 50)
set_value(move_node, "bSweep", "true")
connect(mul_dt, "ReturnValue", move_node, "DeltaLocation")
connect(entry_pin.get_owning_node(), "then", move_node, "execute")

# 左右视觉翻转 (玩家在右则翻转 X=-0.45，玩家在左则 X=0.45)
is_right = fn(ed, "/Script/Engine.KismetMathLibrary.Greater_DoubleDouble", 1360, -100)
connect(break_v, "X", is_right, "A")
set_value(is_right, "B", 0.0)

sel_scale = fn(ed, "/Script/Engine.KismetMathLibrary.SelectFloat", 1580, -100)
set_value(sel_scale, "A", -0.45)
set_value(sel_scale, "B", 0.45)
connect(is_right, "ReturnValue", sel_scale, "bPickA")

scale_vec = fn(ed, "/Script/Engine.KismetMathLibrary.MakeVector", 1800, -100)
...
set_scale = fn(ed, "/Script/Engine.SceneComponent.SetRelativeScale3D", 2050, -150)
...
# 4. 在 EventGraph 中将 ReceiveTick 连接到 ChasePlayer
event_graph = bplib.find_event_graph(bp)
ed_event = unreal.BlueprintGraphEditor.get_graph_editor(event_graph)

tick_node = ed_event.find_event_node("ReceiveTick")
call_chase = fn(ed_event, f"{bp.generated_class().get_path_name()}:ChasePlayer", 300, 0)
connect(tick_node, "then", call_chase, "execute")
connect(tick_node, "DeltaSeconds", call_chase, "DeltaSeconds")
...
unreal.log(f"ZombieWalker AI Chase Build Complete: compiled={compiled}, saved={saved}")

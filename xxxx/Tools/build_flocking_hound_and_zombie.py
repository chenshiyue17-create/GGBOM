# -*- coding: utf-8 -*-
"""
build_flocking_hound_and_zombie.py
工业级 2D 怪物群包围槽位（Surround Slot）与防重叠物理排斥全量构建脚本
"""
import unreal
from pathlib import Path

ROOT = Path(unreal.Paths.project_dir()).resolve()
OUT_HOUND = ROOT / "output/flocking_hound_result.txt"
OUT_ZOMBIE = ROOT / "output/flocking_zombie_result.txt"
...
def log(msg):
    unreal.log(f"[FLOCKING_AI] {msg}")
    print(f"[FLOCKING_AI] {msg}", flush=True)
...
def build_hound():
    ...
    # 3. 距离判断 (StopDistance = 75.0 对准包围槽目标点)
    ...
    # 7. 怪与怪反弹排斥 (ReceiveActorBeginOverlap)
    base_x, base_y = 0, 480
    ...
    OUT_HOUND.write_text(f"SAVED: {saved}", encoding="utf-8")
    return saved
...
def build_zombie():
    ...
    # 2. 行尸扇面内侧分流槽位（Surround Slot 计算）：左路(-40)，右路(+40)
    sub_rel = fn("/Script/Engine.KismetMathLibrary.Subtract_VectorVector", 700, 240)
    connect(self_loc, "ReturnValue", sub_rel, "A")
    connect(p_loc, "ReturnValue", sub_rel, "B")
    ...
    mul_slot_off = fn("/Script/Engine.KismetMathLibrary.Multiply_VectorVector", 1580, 240)
    connect(norm_rel_x, "ReturnValue", mul_slot_off, "A")
    set_value(mul_slot_off, "B", "40.0, 0.0, 10.0")

    # 行尸专属目标点 = p_loc + mul_slot_off
    slot_target_loc = fn("/Script/Engine.KismetMathLibrary.Add_VectorVector", 1800, 180)
    connect(p_loc, "ReturnValue", slot_target_loc, "A")
    connect(mul_slot_off, "ReturnValue", slot_target_loc, "B")

    # 3. 距离判断 (StopDistance = 80.0 对准槽位)
    ...
    OUT_ZOMBIE.write_text(f"SAVED: {saved}", encoding="utf-8")
    return saved

def run():
    h_ok = build_hound()
    z_ok = build_zombie()
    log(f"🚀 全量构建结果: Hound={h_ok}, Zombie={z_ok}")

...
"""
全面修复 Boss 战斗全链路：
1. 子弹 BP_ProjectileBase 碰撞配置（开启 ECC_WorldDynamic & ECC_Pawn 的 Overlap，保证必中）
2. Boss BP_Boss_Overlord 蓝图生命值系统、受击飘字、血条自适应单向缩短、死亡连带血条彻底级联销毁
3. MAP_GGBOM_Main 关卡血条几何严密嵌入（Z=0.17, X=0.24, 轴心左对齐，绝不溢出）并清理冲突的 Presenter
"""
import unreal
import traceback
...
def run():
    ...
    # -------------------------------------------------------------
    log("1. 配置子弹 BP_ProjectileBase 碰撞与伤害逻辑...")
    ...
    set_val(spawn_pop, "ActorClass", "Class'/Game/Blueprints/Combat/Feedback/BP_Combat_DamagePop.BP_Combat_DamagePop_C'")
    ...
    # ---------------- 死亡分支 (branch.True): 级联彻底销毁所有血条 Actor + 销毁自身 ----------------
    get_attached = fn("/Script/Engine.Actor.GetAttachedActors", 2320, 880)
    connect(branch, "then", get_attached, "execute")

    # 销毁 4 个附加血条子 Actor
    last_destroy = get_attached
    for i in range(4):
        act_get = fn("/Script/Engine.KismetArrayLibrary.Array_Get", 2560 + i * 260, 880)
        connect(get_attached, "OutActors", act_get, "TargetArray")
        set_val(act_get, "Index", i)

        dest_sub = fn("/Script/Engine.Actor.K2_DestroyActor", 2700 + i * 260, 880)
        connect(last_destroy, "then", dest_sub, "execute")
        connect(act_get, "Item", dest_sub, "self")
        last_destroy = dest_sub

    # 兜底按 Tag 销毁
    get_tag_bars = fn("/Script/Engine.GameplayStatics.GetAllActorsWithTag", 3800, 880)
    set_val(get_tag_bars, "Tag", "BossBarGroup")
    connect(last_destroy, "then", get_tag_bars, "execute")

    last_tag_dest = get_tag_bars
    for i in range(4):
        tag_get = fn("/Script/Engine.KismetArrayLibrary.Array_Get", 4040 + i * 260, 880)
        connect(get_tag_bars, "OutActors", tag_get, "TargetArray")
        set_val(tag_get, "Index", i)

        dest_tag = fn("/Script/Engine.Actor.K2_DestroyActor", 4180 + i * 260, 880)
        connect(last_tag_dest, "then", dest_tag, "execute")
        connect(tag_get, "Item", dest_tag, "self")
        last_tag_dest = dest_tag

    # 最终销毁 Boss 自身
    destroy_boss = fn("/Script/Engine.Actor.K2_DestroyActor", 5300, 880)
    connect(last_tag_dest, "then", destroy_boss, "execute")

    # ---------------- 存活受击分支 (branch.False): 动态自适应血条单向收缩 ----------------
    # Ratio = Clamp(max_zero / 1200.0, 0.0, 1.0)
    ...
    # 精确 ScaleX: Ratio * 0.24, Z = 0.17 严丝合缝内嵌在凹槽内
    ...
    # -------------------------------------------------------------
    # 步骤 3: 治理 MAP_GGBOM_Main 关卡 Actor
    # -------------------------------------------------------------
    ...
    sp_armor_path = "/Game/P01/Imported/Content/Asset/Art/07_UI/01_HUD/Sprites/SP_T_UI_HUD_08_Minimap_Frame"

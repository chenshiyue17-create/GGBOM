...
"""
================================================================================
《GGBOM: 终末医疗兵》枪口挂载对齐、精确 Pitch 飞行朝向与精致比例调优 (修正版)
================================================================================
"""
...
def calibrate_projectile_scale():
    print("🔫 1. 校准子弹比例至 (0.42, 0.42, 0.42)...")
...
def calibrate_player_muzzle_and_angles():
    print("🎮 2. 校准主角枪口挂载点与 Pitch 8 向射击角度...")
    ...
    # 向上子弹: 枪口正前方 (X, 0, Z+45), 角度 Pitch = 90.0 (正向上飞)
    add_up = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 4000, -260); connect(get_loc, "ReturnValue", add_up, "A"); set_value(add_up, "B", "0,0,45")
    rot_up = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 4000, -140); set_value(rot_up, "Pitch", 90.0); set_value(rot_up, "Yaw", 0.0); set_value(rot_up, "Roll", 0.0)
    ...
    # 向下子弹: 枪口正前方 (X, 0, Z-45), 角度 Pitch = -90.0 (正向下飞)
    add_down = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 4220, 60); connect(get_loc, "ReturnValue", add_down, "A"); set_value(add_down, "B", "0,0,-45")
    rot_down = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 4220, 180); set_value(rot_down, "Pitch", -90.0); set_value(rot_down, "Yaw", 0.0); set_value(rot_down, "Roll", 0.0)
    ...
    # 侧向子弹: 挂载在枪口 (X+40, 0, Z), Pitch = 0.0 (水平飞行)
    add_side = fn(ed, "/Script/Engine.KismetMathLibrary.Add_VectorVector", 4220, 360); connect(get_loc, "ReturnValue", add_side, "A"); set_value(add_side, "B", "40,0,0")
    rot_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeRotator", 4220, 480); set_value(rot_side, "Pitch", 0.0); set_value(rot_side, "Yaw", 0.0); set_value(rot_side, "Roll", 0.0)
    trans_side = fn(ed, "/Script/Engine.KismetMathLibrary.MakeTransform", 4440, 400); connect(add_side, "ReturnValue", trans_side, "Location"); connect(rot_side, "ReturnValue", trans_side, "Rotation"); set_value(trans_side, "Scale", "1,1,1")
    ...
    BPLIB.compile_blueprint(bp)
    ASSETS.save_loaded_asset(bp, only_if_is_dirty=False)
    print("✅ 枪口挂载点与 Pitch 8 向射击角度校准完成！")

def main():
    calibrate_projectile_scale()
    calibrate_player_muzzle_and_angles()

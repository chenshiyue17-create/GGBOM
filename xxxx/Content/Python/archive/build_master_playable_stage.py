...
"""
================================================================================
《GGBOM: 终末医疗兵》全量 1:1 概念图战斗 HUD + WASD 全向平滑移动主角总装流水线
================================================================================
"""
...
def log(msg: str):
    unreal.log(f"[GGBOM-MasterPlayable] {msg}")
...
def build_wasd_player_bp() -> unreal.Blueprint:
    log("🔨 构建全功能 WASD 可控主角蓝图 BP_GGBOM_Player...")
    ...
    # A(+X), D(-X), W(+Z), S(-Z) 移动逻辑
    directions = [
        (br_a, 500.0, 0.0, 0.0, -220),
        (br_d, -500.0, 0.0, 0.0, -100),
        (br_w, 0.0, 0.0, 500.0, 20),
        (br_s, 0.0, 0.0, -500.0, 140),
    ]
    ...
    log("✅ BP_GGBOM_Player WASD 移动图表编译成功！")
    return bp

def assemble_master_stage():
    log("🚀 开始全量组装主关卡 MAP_GGBOM_Main (场景 + HUD + WASD 主角)...")
    ...
    # 先编译主角 Pawn
    player_bp = build_wasd_player_bp()
    ...
    if player_cls:
        ...
        if player_pawn:
            player_pawn.set_actor_label("BP_GGBOM_Player_WASD")

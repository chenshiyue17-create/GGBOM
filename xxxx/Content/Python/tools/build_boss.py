# -*- coding: utf-8 -*-
"""
深渊异化领主 Boss (BP_Boss_Overlord) 独立装配脚本
- 移除 ProjectileMovementComponent
- 4 向完整移动行走动画 (Walk: Down, Right, Up, Left)
- 地裂重击技能 (Ground_Slam: Down)
...
for h in handles:
    ...
    if isinstance(obj, unreal.BoxComponent):
        ...
        break
if target_comp:
    target_comp.set_editor_property("box_extent", unreal.Vector(55.0, 90.0, 65.0))
...
# 7. 距离判定：地裂技能 (Dist <= 150) vs 4 向行走 (Dist > 150)
is_near = fn("/Script/Engine.KismetMathLibrary.LessEqual_DoubleDouble", 950, -80)
connect(dist_node, "ReturnValue", is_near, "A"); set_value(is_near, "B", 150.0)
...
# 技能分支 (True): 播放地裂重砸技能动作
set_slam = fn("/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2550, -50)
...
set_walk_r = fn("/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2450, -120)
if fb_walk_right: set_value(set_walk_r, "NewFlipbook", f"PaperFlipbook'{fb_walk_right.get_path_name()}'")
connect(br_right, "then", set_walk_r, "execute"); connect(get_fb, "ReturnValue", set_walk_r, "self")

set_walk_l = fn("/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2450, -20)
if fb_walk_left: set_value(set_walk_l, "NewFlipbook", f"PaperFlipbook'{fb_walk_left.get_path_name()}'")
connect(br_right, "else", set_walk_l, "execute"); connect(get_fb, "ReturnValue", set_walk_l, "self")
...
set_walk_u = fn("/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2450, 100)
if fb_walk_up: set_value(set_walk_u, "NewFlipbook", f"PaperFlipbook'{fb_walk_up.get_path_name()}'")
connect(br_up, "then", set_walk_u, "execute"); connect(get_fb, "ReturnValue", set_walk_u, "self")

set_walk_d = fn("/Script/Paper2D.PaperFlipbookComponent.SetFlipbook", 2450, 200)
if fb_walk_down: set_value(set_walk_d, "NewFlipbook", f"PaperFlipbook'{fb_walk_down.get_path_name()}'")
connect(br_up, "else", set_walk_d, "execute"); connect(get_fb, "ReturnValue", set_walk_d, "self")
...
print(f"BOSS_BUILD_STATUS: compiled={compiled}, saved={saved}")
